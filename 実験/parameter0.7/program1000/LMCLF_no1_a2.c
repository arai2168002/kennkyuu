		...
		if(state[i] == 1){
	    	for(j=0;j<TN;j++){
	    		if(state[j] == 1){
					...
					}else if(rand_memory[i][step[i]] < rand_memory[j][step[j]]){  // m(i)α+Ci*Li<m(j)α+Cj*Lj && m(j)>m(i) --> Ci*Li-Cj*Lj<(m(j)-m(i))α --> α>(Ci*Li-Cj*Lj)/(m(j)-m(i))
	    		...
	    		if(state[k] == 1){
					for(l=0;l<TN;l++){
						if(state[l] == 1){
		    				...
		    				}else if(rand_memory[k][(k==i)?(step[k]+1):step[k]] < rand_memory[l][(l==i)?(step[l]+1):step[l]] ){  // m(k)α+Ck*Lk<m(l)α+Cl*Ll && m(l)>m(k) --> Ck*Lk-Cl*Ll<(m(l)-m(k))α --> α>(Ck*Lk-Cl*Ll)/(m(l)-m(k))
						...
						fprintf(stderr,"%lf <= alpha <= %lf for scheduling task %d and then task %d\n",alphalowermax,alphauppermin,i+1,k+1);
		  				...
		  				if(minmemory>memory){
		    				...
		    				if(alphauppermin<MAX){
		    					...
		    					//alphadiff=((alphalowermax + alphauppermin)/2-alpha)/1000; //αの目標値を(上限+下限)/2として、その値に1/100000だけ近づける
		    				}else{
		    					//alpha=alphalowermax;
		    					if(((keta1+keta2)/2)>0){
		    					...
		    					//alphadiff=(alphalowermax-alpha)/1000; //αの目標値を下限として、その値に1/100000だけ近づける
		    				}
						}
					}		
	    		}
