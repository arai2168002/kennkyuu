...
int main(void) {
	...
	fprintf(stderr, "\n----------------LLF Scheduling in %d-Processor Environment----------------\n", P);
	
	int i = 0, j = 0, k1 = 0, k2 = 0;
	
	pthread_t Task[S];
	pthread_t LLF_scheduler;
	...
	/*スレッドの生成*/
	pthread_create(&LLF_scheduler, NULL, thread_LLF, NULL);

	/*スレッドの停止*/
	pthread_join(LLF_scheduler,NULL);
	...
	fprintf(stderr, "Worst Case Memory Consumption (LLF) = %d\n", Worst_Memory);
	
	/*ログの出力*/
	FILE *fpout_LLF;
	fpout_LLF = fopen("datafile_LLF.txt","a");
	fprintf(fpout_LLF, "%d\t", Worst_Memory);		
	fprintf(fpout_LLF, "%d\r\n", Deadline_Miss);
	fclose(fpout_LLF);
