...
void LMCLF(){
	...
	unsigned long long b=0,a=0; //プロセッサ分のタスク集合
	...
	/*換算レートαの決定*/
    for(b=1;b<2^TN;b++){ //1ステップ目のプロセッサ分のタスク集合
	    for(i=1,c=0;i<TN;i++){
			if(b&2^(i-1)!=0){  /* bのiビット目が1ならば */
				c++;  /* Iの要素数cを数える */
			}
		}
		if(c<=0 || c>P){/* |I|<=0 または|I|>pなら現在のbは無効として次のbを試す */
				continue;
		}
		    
		for(i=0;i<TN;i++){
			alphauppermin=MAX,alphalowermax=0;
			if(b&2^(i-1)!=0 && state[i] == 1){  /* bのiビット目が1ならば */
				for(j=0;j<TN;j++){
					if(b&2^(j-1)!=1 && state[j] == 1){  //bのiビット目が0ならば
				...
				for(a=1;a<2^TN;a++){ //2ステップ目のプロセッサ分のタスク集合
	    			for(k=1,c=0;k<TN;k++){
						if(a&2^(i-1)!=0){  /* aのiビット目が1ならば */
							c++;  /* Iの要素数cを数える */
						}
					}
					if(c<=0 || c>P){/* |I|<=0 または|I|>pなら現在のaは無効として次のbを試す */
					continue;
					}
	    			for(k=0;k<TN;k++){
	      				alphauppermin=alphaupperminsav; alphalowermax=alphalowermaxsav;
	    				if(a&2^(k-1)!=0 && state[k] == 1){ /* aのiビット目が1ならば */
							for(l=0;l<TN;l++){
								if(a&2^(l-1)!=1 && state[l] == 1){  /* aのiビット目が0ならば */
