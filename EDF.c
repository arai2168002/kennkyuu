...
int absolute[S];			//絶対デッドライン格納変数（作業用）
...
void *thread_EDF();						//EDF Schedular

void EDF();  							//デッドラインをソートする関数
...
int main(void) {
	...
	fprintf(stderr, "\n----------------EDF Scheduling in %d-Processor Environment----------------\n", P);
	
	int i = 0, j = 0, k1 = 0, k2 = 0;
	
	pthread_t Task[S];
	pthread_t EDF_scheduler;
	...
	double period_count = 0;						//最小公倍数までのステップで何度リリースされるか
	...
	/*スレッドの生成*/
	pthread_create(&EDF_scheduler, NULL, thread_EDF, NULL);

	/*スレッドの停止*/
	pthread_join(EDF_scheduler,NULL);
	...
	fprintf(stderr, "Worst Case Memory Consumption (EDF) = %d\n", Worst_Memory);
	
	/*ログの出力*/
	FILE *fpout_EDF;
	fpout_EDF = fopen("datafile_EDF.txt","a");
	fprintf(fpout_EDF, "%d\t", Worst_Memory);		
	fprintf(fpout_EDF, "%d\r\n", Deadline_Miss);
	fclose(fpout_EDF);
