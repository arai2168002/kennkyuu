...
void LMCLF(){
	...
	if(setP1num>=P){//探索範囲を狭めたタスク番号によるタスク番号集合の格納
	...
	freeList(setP1);
  	...
  	for(set1=0;set1<calcNumOfCombination(setP1num,P);set1++){
		...
		for(k=TN-1,val2=createList();k>=0;k--){//val2に評価値を格納
			if(state[k]==1){
				//メモリと時間の桁数を合わせることで仮のαを求めそれを用い評価値の計算
				//valketa=MLOG10((task_data[k].WCET - (memberList(k,kumi1)==1)?(step[k]+1):step[k]) * task_data[k].Laxity_Time) - MLOG10(rand_memory[k][(memberList(k,kumi1)==1)?(step[k]+1):step[k]]);
            ...
            if(alphauppermin>0 && alphalowermax < alphauppermin){ //求めたいαが条件を満たしている時
                ...
                for(i=0,s1val=0;i<TN;i++){
                    if(memberList(i,kumi1)==1 && state[i] == 1){ /* set1のiビット目が1ならば */
						...
						NewLaxityjudge=LINER(task_data[i].Laxity_Time);
                ...
                for(k=0,s2val=0;k<TN;k++){
                    if(memberList(k,kumi2)==1 && state[k] == 1){ /* set2のiビット目が1ならば */
						...
						NewLaxityjudge=LINER(task_data[k].Laxity_Time);
